...
workflow whatshap {
  ...
  scatter(smtrcell_num in range(length(sample.smrtcells))) {
    call samtools_common.samtools_index_bam {
      input:
        bam_input = whatshap_haplotag_round1.deepvariant_haplotagged_bam[smtrcell_num],
        pb_conda_image = pb_conda_image
    }
  }

  output {
     Array[IndexedData] whatshap_bams = samtools_index_bam.bam_pair
  }
}